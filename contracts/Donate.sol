...
contract Donate{
    ...
    function addUser(string memory _name, uint256 _target) public  {
      ...
      require(_target != 0, "funding target is invalid ");
      require(toCheck != true, "you already have a donation in place, an account can't have more than one donation running simultaneously");
    ...
    //increment amount
    function incrementAmount(uint256 _amount, address _receiver) public returns(uint256){
    ...
    function sendEther(address _receiver, uint256 _amount) public payable{
      //donate ether
       _amount = msg.value;
